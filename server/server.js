    ...
    if (type === "register") {
      ...
      if (user && bcrypt.compareSync(password, user.password)) {
        ...
        const fileHistory = db
          .prepare(
            "SELECT sender, fileUrl, fileName FROM files ORDER BY timestamp ASC"
          )
          .all();
      ...
      if (socket.authenticated) {
        ...
        const messageToSend = {
          type: "text",
          sender: socket.username, // ensure sender is the authenticated user
          content,
        };
        if (recipientSocket && recipientSocket.readyState === WebSocket.OPEN) {
          // Route the message to the selected recipient only if their WebSocket is open

          recipientSocket.send(JSON.stringify(messageToSend));
        }
        socket.send(JSON.stringify(messageToSend)); // Echo the message back to the sender
        ...
        ).run(socket.username, content, Date.now());
      ...
      if (socket.authenticated) {
        ...
        if (upload) {
          ...
          if (upload.receivedChunks === upload.totalChunks) {
            ...
            db.prepare(
              "INSERT INTO files (sender, fileUrl, fileName, timestamp) VALUES (?, ?, ?, ?)"
            ).run(
